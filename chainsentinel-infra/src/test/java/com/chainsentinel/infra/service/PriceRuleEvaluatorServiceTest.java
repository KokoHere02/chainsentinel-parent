...
class PriceRuleEvaluatorServiceTest {
	...
	@Test
	void shouldTriggerWhenCooldownJustExpired() throws Exception {
		AlertRuleEntity rule = new AlertRuleEntity();
		ReflectionTestUtils.setField(rule, "id", 21L);
		rule.setType(AlertRuleType.PRICE_THRESHOLD);
		rule.setSeverity("HIGH");
		rule.setConditionJson(buildPriceRuleJson("XRP-USDT", "100", 60));
		...
		existing.setRuleId(21L);
		existing.setTargetKey("XRP-USDT");
		existing.setActive(false);
	...
	@Test
	void shouldBlockWhenCooldownNotYetExpired() throws Exception {
		AlertRuleEntity rule = new AlertRuleEntity();
		ReflectionTestUtils.setField(rule, "id", 22L);
		rule.setType(AlertRuleType.PRICE_THRESHOLD);
		rule.setSeverity("HIGH");
		rule.setConditionJson(buildPriceRuleJson("DOGE-USDT", "100", 60));
		...
		existing.setRuleId(22L);
		existing.setTargetKey("DOGE-USDT");
		existing.setActive(false);
		...
		verify(alertEventRepository, times(0)).save(any(AlertEventEntity.class));
		assertEquals(1.0,
			meterRegistry.get("rule_cooldown_block_total")
				.tag("ruleId", "22")
				.tag("type", "PRICE_THRESHOLD")
